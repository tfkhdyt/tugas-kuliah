...
for i = 1:size(masukan, 1)
  ...
  perubahanBobot(i, :) = [deltaW1 deltaW2 deltaW3 deltaB];
end
...
for i = 1:size(perubahanBobot, 1)
  ...
  b = perubahanBobot(i, 4) + inisialisasi(4);

  inisialisasi = [w1 w2 w3 b];
  bobotBaru(i, :) = inisialisasi;
end

net = [];
for i = 1:size(masukan, 1)
  n = (masukan(i, 1) * bobotBaru(4, 1)) + (masukan(i, 2) * bobotBaru(4, 2)) + (masukan(i, 3) * bobotBaru(4, 3)) + bobotBaru(4, 4);
