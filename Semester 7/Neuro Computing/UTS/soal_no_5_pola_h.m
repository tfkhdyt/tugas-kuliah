...
for i = 1:size(masukan, 1)
  ...
  perubahanBobot(i, :) = [deltaW1 deltaW2 deltaW3 deltaW4 deltaB];
end

bobotBaru = [];
inisialisasi = [0 0 0 0 0];
for i = 1:size(perubahanBobot, 1)
  ...
  b = perubahanBobot(i, 5) + inisialisasi(5);

  inisialisasi = [w1 w2 w3 w4 b];
  bobotBaru(i, :) = inisialisasi;
end

net = [];
for i = 1:size(masukan, 1)
  n = (masukan(i, 1) * bobotBaru(5, 1)) + (masukan(i, 2) * bobotBaru(5, 2)) + (masukan(i, 3) * bobotBaru(5, 3)) + (masukan(i, 4) * bobotBaru(5, 4)) + bobotBaru(5, 5);
